...
/**
 * A Class to execute the LdapToSQL conversion from the command line.
 * 
...
public class LdapToSQL {
     ...
     * 
     * <pre>
     * usage: java -Xmx300 -cp convert.jar cl.LdapToSQL [-SU SQLUserName] 
     * -SD Driver -CS Coding Scheme [-LP LdapPassword] [-SP SQLPassword] 
     * -LA LdapAddress [-DC] [-LU LdapUserName] -SS SQLServer -LS LdapService
     * 
     * 
     ...
     * 
     * 
     * Example: java -Xmx300 -cp convert.jar cl.LdapToSQL -SU user 
     * -SD sun.jdbc.odbc.JdbcOdbcDriver -SS &quot;jdbc:odbc:Driver={Microsoft Access Driver (*.mdb)};DBQ=C:\Temp\LexGrid_Lite.mdb&quot; 
     * -LA ldap://mir04.mayo.edu:31900 -LU cn=HL7User,dc=Users,service=userValidation,dc=LexGrid,dc=org -LP terminology 
    ...
    public static void main(String[] args) throws Exception {
        ...
        String ldapUrl = options.getOptionValue("LA");

        String ldapHost = ldapUrl.substring("ldap://".length(), ldapUrl.lastIndexOf(':'));
        String tempa = ldapUrl.substring(ldapUrl.lastIndexOf(':') + 1);
        if (tempa.endsWith("/") || tempa.endsWith("\\")) {
            tempa = tempa.substring(0, tempa.length() - 1);
        }

        int ldapPort = Integer.parseInt(tempa);

        LexGridLDAP in = new LexGridLDAP(options.getOptionValue("LU"), options.getOptionValue("LP"), ldapHost,
                ldapPort, options.getOptionValue("LS"));
        ...
        convertOptions.add(new edu.mayo.informatics.lexgrid.convert.formats.Option(
                edu.mayo.informatics.lexgrid.convert.formats.Option.ENFORCE_INTEGRITY, new Boolean(!options
                        .hasOption("DC"))));

        ConversionLauncher.startConversion(in, out, new String[] { options.getOptionValue("CS") }, convertOptions,
                director);
