...
public class FMA2EMFDynamicMapHolders {
    ...
    public boolean processFMA(CodingScheme csclass, KnowledgeBase kb) {
        ...
        try {
            ...
            properties_ = FMA2EMFStaticMapHolders.getFixedProperties();

            if (properties_ == null)
                properties_ = new Vector();

            associations_ = FMA2EMFStaticMapHolders.getFixedAssociations();
            ...
            if (testing) {
                ...
                for (int i = 0; i < FMA2EMFConstants.topClasses.length; i++) {
                    System.out.println("Processing Branch -->" + FMA2EMFConstants.topClasses[i]);
                    Cls topCls = kb_.getCls(FMA2EMFConstants.topClasses[i]);
                    loadConcept(topCls);
                }
                ...
                if (o instanceof Slot) {
                    ...
                    if (!slotName.startsWith(":")) {
                        ...
                        if (restricted) {
                            ...
                            if (!FMA2EMFUtils.isNull(slotType)) {
    ...
    private String getSlotTypeFromSlot(Slot slot) {
        ...
        if (slot != null) {
            ...
            if ((FMA2EMFConstants.SLOT_FMA_CONCEPT_CODE.equals(slotName))
                    || (FMA2EMFConstants.SLOT_NAME.equals(slotName))
                    || (FMA2EMFConstants.SLOT_DEFINITION.equals(slotName)))
                return type;

            if ((FMA2EMFConstants.SLOT_FMA_PREFERRED_NAME.equals(slotName))
                    || (FMA2EMFConstants.SLOT_FMA_SYNONYM.equals(slotName))
                    || (FMA2EMFConstants.SLOT_FMA_NON_ENG_EQUIV.equals(slotName))
                    || (FMA2EMFConstants.SLOT_FMA_OTHER_ENG_EQUIV.equals(slotName))) {
                type = FMA2EMFConstants.SLOT_TYPE_PRESENTATION;
            } else {
                ValueType vT = slot.getValueType();

                if ((ValueType.CLS.equals(vT)) || (ValueType.INSTANCE.equals(vT)))
                    type = FMA2EMFConstants.SLOT_TYPE_ASSOCIATION;
                else
                    type = FMA2EMFConstants.SLOT_TYPE_PROPERTY;
            }
        }
                        ...
                        while (itr.hasNext()) {
                            ...
                            if (val != null) {
                                if (("authority".equalsIgnoreCase(slotName)) || ("Source".equalsIgnoreCase(slotName))) {
                                    value = FMA2EMFUtils.toSourceFormat(val);
                                } else
                                    value = val;
                            }
    ...
    private void addSlotDetailsToConcept(Object concept, Concept con, Slot slot, String slotType) {
        ...
        if ((FMA2EMFConstants.SLOT_FMA_CONCEPT_CODE.equals(slotName)) || (FMA2EMFConstants.SLOT_NAME.equals(slotName))
                || (FMA2EMFConstants.SLOT_DEFINITION.equals(slotName)))
            return;

        if (FMA2EMFConstants.SLOT_TYPE_PRESENTATION.equals(slotType)) {
            addPresentationAttribute(concept, con, slot);
        } else if (FMA2EMFConstants.SLOT_TYPE_PROPERTY.equals(slotType)) {
            addPropertyAttribute(concept, con, slot);
        } else if (FMA2EMFConstants.SLOT_TYPE_ASSOCIATION.equals(slotType)) {
    ...
    private String getConceptNameFromInstance(Instance concept) {
        ...
        try {
            if (concept != null) {
                ...
                Slot idSlot = kb_.getSlot(FMA2EMFConstants.SLOT_FMA_PREFERRED_NAME);
                if (concept.hasOwnSlot(idSlot)) {
                    Instance inst = (Instance) concept.getOwnSlotValue(idSlot);
                    conCode = (String) inst.getOwnSlotValue(kb_.getSlot(FMA2EMFConstants.SLOT_NAME));
                } else {
                    try {
                        conCode = (String) concept.getOwnSlotValue(kb_.getSlot(FMA2EMFConstants.SLOT_NAME));
                    } catch (Exception e) {
                    }

                    if (FMA2EMFUtils.isNull(conCode))
                        conCode = FMA2EMFUtils.toNMToken(concept.getName());
                }
    ...
    private String getConceptCodeFromCls(Cls concept) {
        ...
        try {
            if (concept != null) {
                Slot idSlot = kb_.getSlot(FMA2EMFConstants.SLOT_FMA_CONCEPT_CODE);
                if (concept.hasOwnSlot(idSlot)) {
                    conCode = (String) concept.getOwnSlotValue(idSlot);
                } else {
                    idSlot = kb_.getSlot(FMA2EMFConstants.SLOT_FMA_PREFERRED_NAME);
                    if (concept.hasOwnSlot(idSlot)) {
                        Instance inst = (Instance) concept.getOwnSlotValue(idSlot);
                        conCode = (String) inst.getOwnSlotValue(kb_.getSlot(FMA2EMFConstants.SLOT_NAME));
                    } else {
            ...
            if (concept != null) {
                ...
                if (concept.hasOwnSlot(idSlot)) {
                    ...
                    if ((io != null) && (io instanceof Instance)) {
                        ...
                        Slot nameSlot = kb_.getSlot(FMA2EMFConstants.SLOT_NAME);
    ...
    private void modifyPresentationAttribute(Concept con, String conceptName, String slotName, String value) {
        try {
            if ((con == null) || (FMA2EMFUtils.isNull(conceptName)) || (FMA2EMFUtils.isNull(slotName))
                    || (FMA2EMFUtils.isNull(value)))
                return;
                            ...
                            if ("Abbreviation".equalsIgnoreCase(slotName)) {
                                ...
                                if (prs.getValue().getContent().equals(conceptName)) {
                                    ...
                                    if ("Language".equalsIgnoreCase(slotName)) {
                                        ...
                                        if (FMA2EMFUtils.isNull(prs.getLanguage())) {
    ...
    private void addPropertyAttribute(Object concept, Concept con, Slot slot) {
        try {
            String property = FMA2EMFUtils.toNMToken(slot.getName());
            ...
            if (slotCount > 0) {
                ...
                if (valVector.size() > 0) {
                    ...
                    if (createdNew) {
                            ...
                            if (aliases != null) {
                                ...
                                while (itr.hasNext()) {
                                    ...
                                    if ((o != null) && (o instanceof String)) {
                                        ...
                                        associationsAliases_.add(relation + FMA2EMFConstants.DELIM + alias);
                                    }
                                }
                            }
                        }
                        ...
                        if ((invSlot != null) && (!FMA2EMFUtils.isNull(invSlotName))) {
                            if (invSlot.hasOwnSlot(kb_.getSlot("slot synonym"))) {
                                ...
                                if (aliases2 != null) {
                                    ...
                                    while (itr.hasNext()) {
                                        ...
                                        if ((o != null) && (o instanceof String)) {
                                            String alias = (String) o;
                                            associationsAliases_.add(invSlotName + FMA2EMFConstants.DELIM + alias);
                                        }
                                    }
                                }
                            }
    ...
    private String getCorrectDataType(ValueType vT) {
        if (ValueType.ANY.equals(vT))
            return FMA2EMFConstants.PROT_ANY;

        if (ValueType.BOOLEAN.equals(vT))
            return FMA2EMFConstants.PROT_BOOLEAN;

        if (ValueType.CLS.equals(vT))
            return FMA2EMFConstants.PROT_CLS;

        if (ValueType.FLOAT.equals(vT))
            return FMA2EMFConstants.PROT_FLOAT;

        if (ValueType.INSTANCE.equals(vT))
            return FMA2EMFConstants.PROT_INSTANCE;

        if (ValueType.INTEGER.equals(vT))
            return FMA2EMFConstants.PROT_INTEGER;

        if (ValueType.STRING.equals(vT))
            return FMA2EMFConstants.PROT_STRING;

        if (ValueType.SYMBOL.equals(vT))
            return FMA2EMFConstants.PROT_SYMBOL;
    ...
    private Comment[] getComments(Cls concept) {
        try {
            Slot cSlt = kb_.getSlot(FMA2EMFConstants.SLOT_COMMENT);
                ...
                for (int i = 0; i < properties_.size(); i++) {
                    ...
                    if (!FMA2EMFUtils.isNull(prp)) {
                        ...
                        if ((!FMA2EMFConstants.PROPERTY_COMMENT.equals(prp))
                                ...
                                && (!FMA2EMFConstants.PROPERTY_INSTRUCTION.equals(prp))
                                && (!FMA2EMFConstants.PROPERTY_TEXTPRESENTATION.equals(prp))) {
                            suppProp.setUri(FMA2EMFUtils.getWithFMAURN(prp));
                        }
            ...
            if (total > 0) {
                ...
                if (assocAliasSize > 0) {
                    for (int j = assocSize; j < total; j++) {
                        ...
                        String[] ali = sA2.split(FMA2EMFConstants.DELIM);
