...
public class EMF2OBO {
    ...
    public EMF2OBO(CodingScheme codingScheme, LgMessageDirectorIF messages) {
    ...
    String generateOBOContent() {
        ...
        for (Iterator items = entities.getEntity().iterator(); items.hasNext();) {
            Entity codedEntry = (Entity) items.next();
            String entityCode = "";
            if (codedEntry != null && codedEntry.getEntityCode() != null)
                entityCode = codedEntry.getEntityCode();
            if (!"@".equals(entityCode) || !"@@".equals(entityCode))
                str += generateOBOTerm(codedEntry);
        }
    ...
    String generateOBOTerm(Entity codedEntry) {
        ...
        if (property != null) {
            str += "def: \"" + property.getValue().getValue() + "\"" + generateSource(property) + lineReturn;
        }
        // OBO comment
        property = EntitiesUtil.resolveProperty(codedEntry, OBO2EMFConstants.PROPERTY_COMMENT);
        if (property != null) {
            str += "comment: " + property.getValue().getValue() + lineReturn;
        }
        // OBO subset
    ...
    String generateSynonyms(Entity codedEntry) {
        ...
        for (Iterator items = list.iterator(); items.hasNext();) {
            ...
            str += "synonym: " + presentation.getValue().getValue() + " " + presentation.getDegreeOfFidelity()
    ...
    String generateSource(Property property) {
        String str = "";
        List sources = property.getSource();
    ...
    String generateSubset(Entity codedEntry) {
        ...
        for (Iterator items = list.iterator(); items.hasNext();) {
            ...
            str += "subset: " + property.getValue().getValue();
            str += lineReturn;
        }
        return str;
