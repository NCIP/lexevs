...
import edu.mayo.informatics.lexgrid.convert.directConversions.claml.config.ClaMLConfig;
	...
	public class ModifierClassRubricProcessorImpl implements RubricProcessor{

		ClaML clamlXML_;
		CodingScheme schemeCS_;
		ModifierClass modifierClassCS_;
		LgMessageDirectorIF messages_;
		ClaMLConfig config_;
		
		
		public ModifierClassRubricProcessorImpl(ClaML clamlXML, CodingScheme schemeCS, ModifierClass modifierClassCS, ClaMLConfig config, LgMessageDirectorIF messages){
			this.clamlXML_ = clamlXML;
			this.schemeCS_ = schemeCS;
			this.modifierClassCS_ = modifierClassCS;
		...
		private void processReference(Property property, Reference reference){
			...
			try {
				...
				as.setSourceEntityCode(modifierClassCS_.getCode());
				as.setSourceEntityCodeNamespace(this.schemeCS_.getCodingSchemeName());
				...
				for(String target : targetCodes){
					...
					at.setTargetEntityCodeNamespace(this.schemeCS_.getCodingSchemeName());
					RelationsUtil.subsume(as, at);
				}
			} catch (Exception e) {
				messages_.warn(" Error: " + e.getMessage());
			}
		}
