...
public class RadLex2LGDynamicMapHolders {
...
//    private List allCodedEntries_ = null;
    ...
    public boolean processRadlex(CodingScheme csclass, KnowledgeBase kb, LgMessageDirectorIF messages) {
        ...
        try {
            ...
            properties_ = RadLex2LGStaticMapHolders.getFixedProperties();

            if (properties_ == null)
                properties_ = new Vector();

            associations_ = RadLex2LGStaticMapHolders.getFixedAssociations();
            ...
            hasSubTypeAssocClass_.setAssociationName(RadLex2LGConstants.ASSOCIATION_HASSUBTYPE);
            
            hasSubTypeAssocEntityClass_ = new AssociationEntity();
            hasSubTypeAssocEntityClass_.setEntityCode(RadLex2LGConstants.ASSOCIATION_HASSUBTYPE);
            hasSubTypeAssocEntityClass_.setForwardName(RadLex2LGConstants.ASSOCIATION_HASSUBTYPE);
            hasSubTypeAssocEntityClass_.setReverseName(RadLex2LGConstants.ASSOCIATION_ISA);
            ...
            associationHash_.put(RadLex2LGConstants.ASSOCIATION_HASSUBTYPE, hasSubTypeAssocClass_);
            associationEntityHash_.put(RadLex2LGConstants.ASSOCIATION_HASSUBTYPE, hasSubTypeAssocEntityClass_);
            associationRevHash_.put(RadLex2LGConstants.ASSOCIATION_ISA, hasSubTypeAssocClass_);
            ...
            if (testing) {
                ...
                for (int i = 0; i < RadLex2LGConstants.topClasses.length; i++) {
                    // System.out.println("Processing top node -->" +
                    // RadLex2EMFConstants.topClasses[i]);
                    Cls topCls = kb_.getCls(RadLex2LGConstants.topClasses[i]);
                    loadConcept(topCls);
                }
                            ...
                            for (Iterator itr2 = refs.iterator(); itr2.hasNext();)
                            {
                                ...
                                for (Iterator itr3 = domains.iterator(); itr3.hasNext();)
                                {
                                    ...
                                    if (ob3 instanceof Cls)
                                    {
                                        ...
                                        if (!RadLex2LGUtils.isNull(sourceCode))
                                            sourceClses.addElement(sourceCode);
                                    }
                                }

                                String relation2Inst = "has_" + RadLex2LGUtils.toNMToken(slotLGConcept.getEntityDescription().getContent());
    ...
    private Concept createCoceptFromSlot(Slot slot)
    {
        ...
        if (!RadLex2LGUtils.isNull(description)) {
            EntityDescription enDesc = new EntityDescription();
            enDesc.setContent(description);
            con.setEntityDescription(enDesc);
        }
            ...
            while (itr.hasNext()) {
                ...
                if (o instanceof Slot) {
                    ...
                    if (!slotName.startsWith(":")) {
                        ...
                        if (!RadLex2LGUtils.isNull(slotType))
    ...
    private String getSlotTypeFromSlot(Slot slot) 
    {
        ...
        if (slot != null) {
            ...
            {
                if ((RadLex2LGConstants.SLOT_Radlex_PREFERRED_NAME.equals(slotName))
                 ||(RadLex2LGConstants.SLOT_Radlex_SYNONYM.equals(slotName))
                 ||(RadLex2LGConstants.SLOT_Radlex_SYNONYM_OF.equals(slotName))
                ) 
                {
                    type = RadLex2LGConstants.SLOT_TYPE_PRESENTATION;
                } 
                else 
                {
                    ...
                    if ((ValueType.CLS.equals(vT)) || (ValueType.INSTANCE.equals(vT)))
                        type = RadLex2LGConstants.SLOT_TYPE_ASSOCIATION;
                    else
                        type = RadLex2LGConstants.SLOT_TYPE_PROPERTY;
                }
            }
        }
    ...
    private void addParentChildRelationship(String parentConceptcode, String childConceptCode) 
    {
        try
        {
            if ((RadLex2LGUtils.isNull(parentConceptcode))||
               (RadLex2LGUtils.isNull(childConceptCode)))
            {
                return;
            }
    ...
    private void addSlotDetailsToConcept(Object concept, Concept con, Slot slot, String slotType) {
        ...
        if (!toSkipThisSlot(slotName)) {
            if (RadLex2LGConstants.SLOT_TYPE_PRESENTATION.equals(slotType)) {
                addPresentationAttribute(concept, con, slot);
            } else if (RadLex2LGConstants.SLOT_TYPE_PROPERTY.equals(slotType)) {
                addPropertyAttribute(concept, con, slot);
            } else if (RadLex2LGConstants.SLOT_TYPE_ASSOCIATION.equals(slotType)) {
    ...
    private boolean toSkipThisSlot(String slotName) {
        if ((RadLex2LGConstants.SLOT_Radlex_CONCEPT_CODE.equals(slotName))
                || (RadLex2LGConstants.SLOT_NAME.equals(slotName))
                || (RadLex2LGConstants.SLOT_HASSUBTYPE.equals(slotName))
                //|| (RadLex2EMFConstants.SLOT_Radlex_SYNONYM_OF.equals(slotName))
                || (RadLex2LGConstants.SLOT_DEFINITION.equals(slotName)))
            return true;
    ...
    private String getConceptCodeFromCls(Cls concept) {
        ...
        try {
            if (concept != null) {
                Slot idSlot = kb_.getSlot(RadLex2LGConstants.SLOT_Radlex_CONCEPT_CODE);
                if (concept.hasOwnSlot(idSlot)) {
                    conCode = (String) concept.getOwnSlotValue(idSlot);
                } else {
                    idSlot = kb_.getSlot(RadLex2LGConstants.SLOT_Radlex_PREFERRED_NAME);
                    if (concept.hasOwnSlot(idSlot)) {
                        Instance inst = (Instance) concept.getOwnSlotValue(idSlot);
                        conCode = (String) inst.getOwnSlotValue(kb_.getSlot(RadLex2LGConstants.SLOT_NAME));
                    } else {
                        conCode = RadLex2LGUtils.toNMToken(concept.getName());
                    }
                }
            }
        } catch (Exception e) {
            ...
            if (concept != null) {
                ...
                if (concept.hasOwnSlot(idSlot)) {
                    ...
                    if (concept.hasOwnSlot(idSlot)) {
                        ...
                        try {
                            conCode = (String) concept.getOwnSlotValue(kb_.getSlot(RadLex2LGConstants.SLOT_NAME));
                        } catch (Exception e) {
                        }

                        if (RadLex2LGUtils.isNull(conCode))
                            conCode = RadLex2LGUtils.toNMToken(concept.getName());
                    }
                }
            }
    ...
    private String getEntityDescription(Cls concept) {
        ...
        try {
            if (concept != null) {
                Object io = null;
                Slot idSlot = kb_.getSlot(RadLex2LGConstants.SLOT_Radlex_PREFERRED_NAME);
                if (concept.hasOwnSlot(idSlot))
                    io = concept.getOwnSlotValue(idSlot);
                else {
                    idSlot = kb_.getSlot(RadLex2LGConstants.SLOT_Radlex_SYNONYMS_SLOT);
    ...
    private void addPresentationToConcept(Concept con, String value, String slotName, String code) {
            ...
            while (itr.hasNext()) {
                ...
                if (ob instanceof Presentation) {
                    if (value.equals(((Presentation) ob).getValue().getContent())) {
                        foundSome = true;
                        if (slotName.equals(RadLex2LGConstants.SLOT_Radlex_PREFERRED_NAME))
        ...
        if (!foundSome) 
        {
            ...
            if (RadLex2LGUtils.isNull(slotName))
                tp.setPropertyName(RadLex2LGConstants.PROPERTY_TEXTPRESENTATION);
            else
            {
                if ((slotName.startsWith("Synonym"))||(slotName.startsWith("synonym")))
                    tp.setPropertyName(RadLex2LGConstants.SLOT_Radlex_SYNONYM);
                else
                    tp.setPropertyName(slotName);
            }
            
            if (RadLex2LGUtils.isNull(code))
            {
                //TODO: Deepak:Find if the slot's code with RadLex_ID exists and then use its code if present

                tp.setPropertyId(RadLex2LGConstants.PROPERTY_ID_PREFIX + (++propertyCounter));
            }
            else
    ...
    private void addPresentationAttribute(Object concept, Concept con, Slot slot) {
        ...
        try {
            ...
            if (RadLex2LGUtils.isNull(value))
                return;

            if ((!RadLex2LGConstants.SLOT_Radlex_SYNONYM.equals(slotName)) &&
                    (!RadLex2LGConstants.SLOT_Radlex_SYNONYM_OF.equals(slotName)))
            {
                ...
                if ((slotValues != null) && (slotValues.size() > 0)) {
                    ...
                    while (itr.hasNext()) {
                        ...
                        if (o instanceof Instance) {
                            ...
                            Slot synonym_slot = kb_.getSlot(RadLex2LGConstants.SLOT_Radlex_SYNONYMS_SLOT);
    ...
    private void addPropertyAttribute(Object concept, Concept con, Slot slot) {
        try {
          ...
          //TODO: Deepak:Find if the slot's code with RadLex_ID exists and then use its code if present
            pc.setPropertyId(RadLex2LGConstants.PROPERTY_ID_PREFIX + (++propertyCounter));
    ...
    private void addAssociationBetweenSourcesAndInstances(String sourceCode, org.LexGrid.concepts.Instance inst, String relation)
    {
        ...
        if ((RadLex2LGUtils.isNull(sourceCode))||
            (RadLex2LGUtils.isNull(relation))||
            (inst == null))
            return;
    ...
    private void addAssociationAttribute(Object concept, Concept con, Slot slot) {
        try {
            ...
            if (RadLex2LGUtils.isNull(relation))
                return;
            ...
            try {
                ...
                if (associationHash_.containsKey(relation)) {
                    ...
                    if (associationRevHash_.containsKey(relation)) {
                        ...
                        if (!RadLex2LGUtils.isNull(originalRevName))
                ...
                if (slotCount > 0) 
                {
                    ...
                    if ((slotValues != null) && (slotValues.size() > 0)) {
                        ...
                        while (itr.hasNext()) {
                            ...
                            if (!RadLex2LGUtils.isNull(stringValue)) {
                                if ((ValueType.CLS.equals(vT)) || (ValueType.INSTANCE.equals(vT))) {
                                    AssociationTarget aT = new AssociationTarget();
    ...
    private String getCorrectDataType(ValueType vT) {
        if (ValueType.ANY.equals(vT))
            return RadLex2LGConstants.PROT_ANY;

        if (ValueType.BOOLEAN.equals(vT))
            return RadLex2LGConstants.PROT_BOOLEAN;

        if (ValueType.CLS.equals(vT))
            return RadLex2LGConstants.PROT_CLS;

        if (ValueType.FLOAT.equals(vT))
            return RadLex2LGConstants.PROT_FLOAT;

        if (ValueType.INSTANCE.equals(vT))
            return RadLex2LGConstants.PROT_INSTANCE;

        if (ValueType.INTEGER.equals(vT))
            return RadLex2LGConstants.PROT_INTEGER;

        if (ValueType.STRING.equals(vT))
            return RadLex2LGConstants.PROT_STRING;

        if (ValueType.SYMBOL.equals(vT))
            return RadLex2LGConstants.PROT_SYMBOL;
        ...
        if (ValueType.CLS.equals(vT)) {
            if ((valObj != null) && (valObj instanceof Cls)) {
                ...
                else
                {
                    ...
                    if (RadLex2LGUtils.isNull(result))
    ...
    private Comment[] getComments(Cls concept) {
        try {
            Slot cSlt = kb_.getSlot(RadLex2LGConstants.SLOT_COMMENT);
            String comment1 = (String) concept.getOwnSlotValue(cSlt);

            Slot cmtSlot = kb_.getSlot(RadLex2LGConstants.SLOT_DOCUMENTATION);
            String comment2 = (String) concept.getOwnSlotValue(cmtSlot);
            
            Comment cmt1 = null;
            
            if (!RadLex2LGUtils.isNull(comment1)) 
            {
                cmt1 = new Comment();
                Text txt = new Text();
                txt.setContent(RadLex2LGUtils.removeInvalidXMLCharacters(comment1, concept.getName()));
                cmt1.setValue(txt);
                cmt1.setPropertyName(RadLex2LGConstants.PROPERTY_COMMENT);
                cmt1.setPropertyId(RadLex2LGConstants.PROPERTY_ID_PREFIX + (++propertyCounter));
            }
            
            Comment cmt2 = null;
            
            if (!RadLex2LGUtils.isNull(comment2)) 
            {
                cmt2 = new Comment();
                Text txt = new Text();
                txt.setContent(RadLex2LGUtils.removeInvalidXMLCharacters(comment2, concept.getName()));
                cmt2.setValue(txt);
                cmt2.setPropertyName(RadLex2LGConstants.PROPERTY_COMMENT);
                cmt2.setPropertyId(RadLex2LGConstants.PROPERTY_ID_PREFIX + (++propertyCounter));
            }
    ...
    private Definition[] getDefinitions(Cls concept) {
        try {
            String definition = (String) concept.getOwnSlotValue(kb_.getSlot(RadLex2LGConstants.SLOT_DEFINITION));

            if (definition != null) {
                Definition def = new Definition();
                Text txt = new Text();
                txt.setContent(RadLex2LGUtils.removeInvalidXMLCharacters(definition, concept.getName()));
                def.setValue(txt);
                def.setPropertyName(RadLex2LGConstants.PROPERTY_DEFINITION);
                def.setPropertyId(RadLex2LGConstants.PROPERTY_ID_PREFIX + (++propertyCounter));
        ...
        try {
            if (properties_.size() > 0) {
                for (int i = 0; i < properties_.size(); i++) {
                    ...
                    if (!RadLex2LGUtils.isNull(prp)) {
                        SupportedProperty suppProp = new SupportedProperty();

                        if ((!RadLex2LGConstants.PROPERTY_COMMENT.equals(prp))
                                && (!RadLex2LGConstants.PROPERTY_DEFINITION.equals(prp))
                                && (!RadLex2LGConstants.PROPERTY_INSTRUCTION.equals(prp))
                                && (!RadLex2LGConstants.PROPERTY_TEXTPRESENTATION.equals(prp))) {
                            suppProp.setUri(RadLex2LGUtils.getWithRadlexURN(prp));
                        }
            ...
            if (total > 0) {
                if (assocSize > 0) {
                    for (int i = 0; i < assocSize; i++) {
                        ...
                        if (!RadLex2LGUtils.isNull(sA)) {
                            SupportedAssociation suppAss = new SupportedAssociation();
                            suppAss.setUri(RadLex2LGUtils.getWithRadlexURN(sA));
                            suppAss.setLocalId(RadLex2LGUtils.toNMToken(sA));
                ...
                if (assocAliasSize > 0) {
                    for (int j = assocSize; j < total; j++) {
                        ...
                        String[] ali = sA2.split(RadLex2LGConstants.DELIM);

                        if ((ali != null) && (ali.length > 1)) {
                            if (!RadLex2LGUtils.isNull(ali[1])) {
                                SupportedAssociation suppAss = new SupportedAssociation();
                                suppAss.setUri(RadLex2LGUtils.getWithRadlexURN(ali[0]));
                                suppAss.setLocalId(RadLex2LGUtils.toNMToken(ali[1]));
                                csclass.getMappings().addSupportedAssociation(suppAss);
                            }
                        }
                    }
            ...
            if (srcsSize > 0) {
                if (srcsSize > 0) {
                    for (int i = 0; i < srcsSize; i++) {
                        ...
                        if (!RadLex2LGUtils.isNull(src)) {
                            SupportedSource suppSrc = new SupportedSource();
                            suppSrc.setUri(RadLex2LGUtils.getWithRadlexURN(src));
                            suppSrc.setLocalId(RadLex2LGUtils.toNMToken(src));
            ...
            if (repfSize > 0) {
                if (repfSize > 0) {
                    for (int i = 0; i < repfSize; i++) {
                        ...
                        if (!RadLex2LGUtils.isNull(rep)) {
                            SupportedRepresentationalForm suppRpf = new SupportedRepresentationalForm();
                            suppRpf.setUri(RadLex2LGUtils.getWithRadlexURN(rep));
                            suppRpf.setLocalId(RadLex2LGUtils.toNMToken(rep));
