...
/**
 * GUI that allows loading Text files into LexBIG.
 * 
 * @author <A HREF="mailto:armbrust.daniel@mayo.edu">Dan Armbrust</A>
 * @version subversion $Revision: $ checked in on $Date: $
 */
public class TextLoader extends LoadExportBaseShell {

	public TextLoader(LB_GUI lb_gui) {
		super(lb_gui);
		try {
			...
			Text_Loader loader = (Text_Loader) lb_gui_.getLbs()
					.getServiceManager(null).getLoader("TextLoader");
	...
	private void buildGUI(Shell shell, final Text_Loader loader) {
		...
		Button fileChooseButton = Utility.getFileChooseButton(options, file,
				new String[] { "*.txt", "*.csv" },
				new String[] { "Lexgrid text file (txt, csv)" });
		gd = new GridData(SWT.CENTER);
		gd.widthHint = 60;
		fileChooseButton.setLayoutData(gd);

		Label delimiterLabel = new Label(options, SWT.NONE);
		delimiterLabel.setText("Delimiter (optional):");

		final Text delimiter = new Text(options, SWT.BORDER);
		gd = new GridData();
		gd.horizontalSpan = 2;
		delimiter.setLayoutData(gd);

		Label readDoublesAsTriplesLabel = new Label(options, SWT.None);
		readDoublesAsTriplesLabel.setText("Read Doubles As Triples");

		final Button readDoublesAsTriples = new Button(options, SWT.CHECK);
		readDoublesAsTriples.setSelection(false);
		gd = new GridData(GridData.FILL_HORIZONTAL);
		readDoublesAsTriples.setLayoutData(gd);
		...
		load.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				try {
					...
					if (delimiter.getText().length() > 0) {
						if (delimiter.getText().length() > 1) {
							dialog_
									.showError("Invalid Delimiter",
											"The Delimiter can only be a single character");
							return;
						}
						character = new Character(delimiter.getText().charAt(0));
					}

					setLoading(true);
					loader.load(uri, character, readDoublesAsTriples.getSelection(), false, true);
