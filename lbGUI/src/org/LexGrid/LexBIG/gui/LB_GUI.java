...
public class LB_GUI {
	...
	private void buildCodeSystemComposite(Composite holder) {
		...
		if (isAdminEnabled) {// begin Admin enabled conditional
			...
			Button registerAsSupplement = Utility.makeButton("Register As Supplement",
			        codeSystemComposite_, GridData.VERTICAL_ALIGN_BEGINNING
			        | GridData.FILL_HORIZONTAL);

			registerAsSupplement.setEnabled(true);

			registerAsSupplement.addSelectionListener(new SelectionListener() {
			    ...
			    public void widgetSelected(SelectionEvent arg0) {
			        ...
			        textLabel.setText("Parent: ");

			        final Combo comboDropDown = new Combo(group, SWT.DROP_DOWN | SWT.BORDER);

			        for(TableItem pickListItem : codeSystemsTV_.getTable().getItems()) {
			            comboDropDown.add(((CodingSchemeRendering)pickListItem.getData()).getCodingSchemeSummary().getFormalName());
			        }
			        ...
			        Button register = Utility.makeButton("Register",
			                group, GridData.VERTICAL_ALIGN_BEGINNING
		                    | GridData.FILL_HORIZONTAL);
