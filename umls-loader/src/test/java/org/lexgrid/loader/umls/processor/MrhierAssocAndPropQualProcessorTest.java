...
public class MrhierAssocAndPropQualProcessorTest {
	...
	public void setUp() throws Exception {
		processor = new MrhierAssocAndPropQualProcessor();
		...
		EntityAssnsToEntity rel1 = this.buildEntityAssnsToEntity("code", "code1");
		rel1.setMultiAttributesKey("1");
		EntityAssnsToEntity rel2 = this.buildEntityAssnsToEntity("code1", "code2");
		rel2.setMultiAttributesKey("2");
		EntityAssnsToEntity rel3 = this.buildEntityAssnsToEntity("code2", "code3");
		...
		expect(stagingDao.getCodeFromAui("aui")).andReturn("code");
		expect(stagingDao.getCodeFromAui("aui1")).andReturn("code1");
		expect(stagingDao.getCodeFromAui("aui2")).andReturn("code2");
		expect(stagingDao.getCodeFromAui("aui3")).andReturn("code3");
		
		replay(stagingDao);
		
		processor.setMrconsoStagingDao(stagingDao);
		
		processor.setCodingSchemeIdSetter(new SupportHelpers.TestCodingSchemeNameSetter());	
	...
	public void testPathToRoot() {
		...
		assertTrue(ptr.get(0).equals("code1"));
		assertTrue(ptr.get(1).equals("code2"));
		assertTrue(ptr.get(2).equals("code3"));
	}
	...
	private EntityAssnsToEntity buildEntityAssnsToEntity(String sourceCode, String targetCode){
		...
		assoc.setCodingSchemeName(new SupportHelpers.TestCodingSchemeNameSetter().getCodingSchemeId());
