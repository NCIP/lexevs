...
public class ResourceManager {
    ...
    // The Lucene query parser to use for parsing the user portion of the query
    private IndexQueryParserFactory parser_;
    ...
    private static ResourceManager preInit(Properties props) {
        if (resourceManager_ == null) {
            try {
                ...
                // make sure that any class that uses a WrappedConnection uses
                // my class loader to find the
                // drivers
                WrappedConnection.setDriverClassLoader(MyClassLoader.instance());
    ...
    /*
     * Register mayo provided extensions. This should only be called once per
     * JVM.
     */
    private static void registerExtensions() throws LBParameterException, LBException {
        // sort extensions
        ...
        try {
            ...
            logger_.warn(meta.getName() + " is not on the classpath or could not be loaded as an Extension.",e);
        }
        ...
        try {
            ...
            logger_.warn(umls.getName() + " is not on the classpath or could not be loaded as an Extension.",e);
        }
    ...
    private void init() throws Exception {
        ...
        // Tell the Lucene indexer to not store everything - set this the
        // other way for debug purposes.
        LuceneLoaderCode.storeLexBIGMinimum = true;
        ...
        while (iterator.hasNext()) {
            ...
            if (!temp.exists() || !temp.isDirectory()) {
                ...
                try {
                    ...
                    logger_.error("Skipping the index location " + location + " because of " + e1.getLogId());
                }
            }
        }

        parser_ = new IndexQueryParserFactory();
    ...
    public boolean isSortAlgorithmValid(String algorithm, SortContext context) {
...

    {
        try {
            if (version == null || version.length() == 0) {
                version = getInternalVersionStringFor(codingScheme, null);
            }
    ...
    public String getInternalVersionStringFor(String externalCodeSystemName, String tag) throws LBParameterException {
        ...
        String key = "internalVersionStringForTag:" + externalCodeSystemName + ":"
                + (tag == null || tag.length() == 0 ? LBConstants.KnownTags.PRODUCTION.toString() : tag);
        ...
        version = registry_.getVersionForTag(urn, (tag == null || tag.length() == 0 ? LBConstants.KnownTags.PRODUCTION
                .toString() : tag));

        if (version == null) {
            if (tag != null && !tag.equals(LBConstants.KnownTags.PRODUCTION.toString())) {
    ...
    public IndexQueryParserFactory getQueryParser() {
        return parser_;
    }

    public LexBIGServiceManager getLexBIGServiceManager(@SuppressWarnings("unused") Object credentials)
            throws LBParameterException {
        // TODO [future at request] validate credentials
        if (lbsm_ == null) {
            lbsm_ = new LexBIGServiceManagerImpl();
        }
        return lbsm_;
    }
    ...
    public String getURNForExternalCodingSchemeName(String externalCodingSchemeName) throws LBParameterException {
        String version = getInternalVersionStringFor(externalCodingSchemeName, null);
