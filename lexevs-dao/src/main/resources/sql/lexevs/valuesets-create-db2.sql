...
COMMENT ON TABLE @PREFIX@valueSetDefinition IS 'SQLWAYS_EVAL# value set definitions. A value set definition can be a simple description with no associated value set entries, or it can consist of one or more definitionEntries that resolve to an enumerated list of entityCodes when applied to one or mo';
...
COMMENT ON TABLE @PREFIX@vsdEntry IS 'SQLWAYS_EVAL# entries of a value set definition. A reference to an entry code, a coding scheme or another value set definition along with the instructions about how the reference is applied. Definition entrys are applied in entryOrder, with each succes';
...
COMMENT ON COLUMN @PREFIX@vsdEntry.targetToSource           IS 'SQLWAYS_EVAL# is supplied, navigate from entityCode as the association target to the corresponding sources. If transitiveClosure is true and the referenceAssociation is transitive, include all the ancestors in the list rather than just';
...
COMMENT ON COLUMN @PREFIX@vsdEntry.expirationDate           IS 'SQLWAYS_EVAL# that this resource is considered to become inActive.  To be considered active, isActive must be true, and the temporal context of the operation must be less than expirationDate.  If omitted, all temporal contexts are considered to be va';
...
COMMENT ON COLUMN @PREFIX@vsPickList.completeSet                   IS 'SQLWAYS_EVAL# this pick list should represent all of the entries in the value set definition.  Any active entry codes that aren''t in the specific pick list entries are added to the end, using the designations identified by the defaultLanguage, default';
...
COMMENT ON COLUMN @PREFIX@vsPickList.defaultLanguage               IS 'SQLWAYS_EVAL# of the language that is used to generate the text of this pick list if not otherwise specified. Note that this language does NOT necessarily have any coorelation with the language of a pickListEntry itself or the language of the targ';
...
COMMENT ON COLUMN @PREFIX@vsPickList.expirationDate                IS 'SQLWAYS_EVAL# that this resource is considered to become inActive.  To be considered active, isActive must be true, and the temporal context of the operation must be less than expirationDate.  If omitted, all temporal contexts are considered to be va';
...
ALTER TABLE @PREFIX@valueSetDefinition ADD CONSTRAINT PK_VSDEF 
PRIMARY KEY(valueSetDefGuid);

ALTER TABLE @PREFIX@vsdEntry ADD CONSTRAINT PK_VSDENTRY 
PRIMARY KEY(vsdEntryGuid);

ALTER TABLE @PREFIX@vsEntryState ADD CONSTRAINT PK_VSENSTATE 
PRIMARY KEY(entryStateGuid);

ALTER TABLE @PREFIX@vsMultiAttrib ADD CONSTRAINT PK_VSMULATT
...
ALTER TABLE @PREFIX@vsPropertyMultiAttrib ADD CONSTRAINT PK_VSPROMULAT 
...
ADD CONSTRAINT UQ_VSDEN_VSDEFGUID UNIQUE(valueSetDefGuid,vsdEntryGuid);
...
ADD CONSTRAINT UQ_VSPLEN_VSPKGUID UNIQUE(vsPickListGuid,plEntryId);
...
--SQLWAYS_EVAL# Key Constraints 
ALTER TABLE @PREFIX@valueSetDefinition ADD CONSTRAINT FK_VSD_RELGUID 
FOREIGN KEY(releaseGuid) REFERENCES @PREFIX@systemRelease(releaseGuid);

ALTER TABLE @PREFIX@vsdEntry ADD CONSTRAINT FK_VSDENTRY_VSDEF 
FOREIGN KEY(valueSetDefGuid) REFERENCES @PREFIX@valueSetDefinition(valueSetDefGuid)
ON DELETE CASCADE;

ALTER TABLE @PREFIX@vsEntryState ADD CONSTRAINT FK_VSES_PRENSTGUID 
FOREIGN KEY(prevEntryStateGuid) REFERENCES @PREFIX@vsEntryState(entryStateGuid)
ON DELETE CASCADE;

ALTER TABLE @PREFIX@vsPickList ADD CONSTRAINT FK_VSDPL_RELGUID 
FOREIGN KEY(releaseGuid) REFERENCES @PREFIX@systemRelease(releaseGuid);

ALTER TABLE @PREFIX@vsPLEntry ADD CONSTRAINT FK_VSPLEN_VSPKLST 
FOREIGN KEY(vsPickListGuid) REFERENCES @PREFIX@vsPickList(vsPickListGuid)
ON DELETE CASCADE;

ALTER TABLE @PREFIX@vsPropertyMultiAttrib ADD CONSTRAINT FK_VSPROMUL_VSPRO 
