...
public class VersionableEventAuthoringServiceTest extends LexEvsDbUnitTestBase {
	...
	@Test
	public void testLoadSystemRelease() throws Exception {
		
		init();
		
		URI sourceURI = new File("src/test/resources/Automobiles2010.xml").toURI();
        
        org.exolab.castor.xml.Unmarshaller um = new org.exolab.castor.xml.Unmarshaller(
                SystemRelease.class);
        SystemRelease systemRelease = (SystemRelease) um.unmarshal(new InputStreamReader(sourceURI.toURL()
                .openConnection().getInputStream()));
        
        service.loadSystemRelease(systemRelease);
	}
	
	public void testLoadRevision() throws Exception {
