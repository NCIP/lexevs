...
import org.LexGrid.valueDomains.types.DefinitionOperator;
 ...
 */
public class VDEntryServices extends VDBaseService{
	
	private VDEntryTypeServices entryTypeServices_;
	
	protected VDEntryServices(JDBCBaseService anchorService)
			throws ServiceInitException {
		super(anchorService);
	}
	
	public VDEntryServices(HomeServiceBroker broker, JDBCConnectionDescriptor desc,
	...
	public void insert(ManagedObjIF obj, int foreignEntryId) throws LBException, Exception
	{
		...
		try
		{
			...
			insertIntoVDEntryn = getKeyedInsertStatement(VDConstants.TBL_VD_ENTRY);
			...
			String operator = de.getOperator() == null ? DefinitionOperator.OR.toString() : de.getOperator().getLiteral();
	...
	public void remove(int foreignEntryId) throws ObjectNotFoundException, SQLException
	{
		...
		try
		{
			deleteFromVDEntry = checkOutPreparedStatement(
					" DELETE FROM " + VDConstants.TBL_VD_ENTRY + 
					" WHERE " + SQLTableConstants.TBLCOL_FOREIGNENTRYID + " = ? ");
