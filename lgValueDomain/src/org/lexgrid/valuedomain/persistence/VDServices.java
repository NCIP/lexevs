 ...
 */
public class VDServices extends VDBaseService {

	private VDEntryServices vdEntryServices_;
	private VDPropertyServices propertyServices_;
	private VDEntryTypeServices entryTypeServices_;
	private VDEntryStateService entryStateService_;
	private VDMappingServices mappingServices_;
	private SystemReleaseServices systemReleaseServices_;

	protected VDServices(JDBCBaseService anchorService)
			throws ServiceInitException {
		super(anchorService);
	}

	public VDServices(HomeServiceBroker broker, JDBCConnectionDescriptor desc,
	...
	public ManagedObjIF findByPrimaryKey(Object key) throws FindException {
		...
		try {
			...
			selectFromValueDomain = checkOutPreparedStatement(" SELECT * "
					+ " FROM " + VDConstants.TBL_VALUE_DOMAIN + " WHERE "
	...
	public URI[] findByValueDomainName(Object key) throws FindException {
		...
		try {
			...
			String sql = "SELECT " + SQLTableConstants.TBLCOL_VALUEDOMAINURI
					+ " FROM " + VDConstants.TBL_VALUE_DOMAIN;
	...
	private void getAttributes(ValueDomainDefinition valueDomain, int entryId)
		...
		try {
			selectVDSubProperties = checkOutPreparedStatement(modifySql(" SELECT P1.* "
					+ " FROM " + VDConstants.TBL_PROPERTY + " {AS} P1 " 
					+ " , " + VDConstants.TBL_ENTRY_TYPE + " {AS} P2 "
			...
			selectVDSubProperties.setString(2,
					VDConstants.ENTRY_STATE_TYPE_ATTRIBUTE);
	...
	public void insert(ManagedObjIF obj, String systemReleaseURI, Mappings mappings) throws InsertException,
		...
		try {
			...
			try {
				...
				insertIntoValueDomain = getKeyedInsertStatement(VDConstants.TBL_VALUE_DOMAIN);
				int i = 1;
	...
	public void insertAttributes(ValueDomainDefinition valueDomain,
		...
		try {
			insertIntoProperty = getKeyedInsertStatement(VDConstants.TBL_PROPERTY);
			...
			while (sourceIter.hasNext()) {
				...
				int entryId = getNextEntryId();
				insertEntryType(VDConstants.ENTRY_STATE_TYPE_ATTRIBUTE, entryId);
	...
	protected void initNestedServices() throws ServiceInitException {
		vdEntryServices_ = new VDEntryServices(this);
		propertyServices_ = new VDPropertyServices(this);
		entryTypeServices_ = new VDEntryTypeServices(this);
		entryStateService_ = new VDEntryStateService(this);
	...
	private void removeValueDomain(int entryId) throws SQLException, FindException {
		...
		try {
			...
			deleteFromValueDomain = checkOutPreparedStatement("DELETE FROM " 
					+ VDConstants.TBL_VALUE_DOMAIN + " WHERE "
					+ SQLTableConstants.TBLCOL_ENTRYID + " = ?");
