 ...
 */
public class VDBaseService extends LGBaseService
{
	private static Logger log = Logger.getLogger("convert.SQL");
	
	private VDBaseSQLServices baseSQLServ_;
	
	private static int highEntryId_ = -999;
	private static int entryId_ = 1000;
	
	public VDBaseService(JDBCBaseService anchorService) throws ServiceInitException
    {
        super(anchorService);
        try {
			baseSQLServ_ = new VDBaseSQLServices(getConnectionPool(), "");
	...
	public VDBaseService(HomeServiceBroker broker, JDBCConnectionDescriptor desc,
	...
	public int getNextEntryId() throws LBException, Exception{
		synchronized (VDBaseService.class) {			
			if (entryId_ >= highEntryId_)
			{
				entryId_ = baseSQLServ_.getCurrentEntryId();
				if (entryId_ == 0)
				{
					entryId_ = VDConstants.ENTRYID_INCREMENT;
					highEntryId_ = entryId_ + VDConstants.ENTRYID_INCREMENT;
    ...
    VDBaseSQLServices getVDBaseSQLService(){
    	return baseSQLServ_;
    }
}