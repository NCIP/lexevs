	...
	private void associationMapping() throws LBResourceUnavailableException, Exception{
		...
		while (conIterator.hasNext()) {
			...
			while (localIterator.hasNext()) {
				...
				if (LexRdfMap.isMapped(sourceConRef, ontType_) == true)
					continue;
...
//						prop = model_.getProperty(this.getPropertyUri(supportedAssn.getEntityCodeNamespace(), supportedAssn.getEntityCode()));
					...
					if (prop == null){
						...
						if (LexRdfMap.isMapped(null, supportedAssn.getLocalId(), ontType_) == true) 
							return;
...
//					source.addProperty(prop, this.resolveNamespace(target.getCodeNamespace()) + target.getCode());
					if (this.ontType_.equals(OntologyType.OBO) && prop.getURI().equalsIgnoreCase(RDFS.subClassOf.getURI())) {
					...
					else if(this.ontType_.equals(OntologyType.UMLS) && prop.getURI().equalsIgnoreCase(RDFS.subClassOf.getURI())) {
...
//						System.out.println("process source: " + sourceConRef.getCode() + "-> " + target.getCode());
	...
	private String getPropertyUri(String ns, String code) throws LBException {
		if (this.ontType_.equals(OntologyType.OWLRDF))
			return this.resolveNamespace(ns) + code;
		else {
			if (LexRdfMap.get(code, ontType_) != null)
				return LexRdfMap.get(code, ontType_).getURI();
			else
				return null;
		}
	}
...
//    }
	...
	private void addOntProperty(AssociationEntity entity) throws LBException, SecurityException, IllegalArgumentException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
		// if it maps to owl/rdf properties, we don't need to add it
		if (LexRdfMap.isMapped(entity, ontType_) == true) 
			return;
...
//			ontProperty.addRange(model_.getResource(id));
	...
	public void toTripleStore(CodingScheme cs, CodedNodeGraph baseCng,
			CodedNodeSet cns, Writer writer, LgMessageDirectorIF messenger, OntologyType ontType){
		Model baseModel = this.getBaseModel(NS_URI);
...
//		store_.getTableFormatter().truncate(); // for development only
	...
	public void toOwlOntology(String iri) throws ClassNotFoundException {
		...
		try {
			...
			rdfw.setProperty(LexRdfConstants.XML_BASE, NS_URI);
			rdfw.setProperty(LexRdfConstants.RELATIVE_URIS, "");
			FileOutputStream f = new FileOutputStream(outputFile_);
			rdfw.write(localModel, f, NS_URI);
			// model.write(f, JenaConstants.RDFXML_ABBREV, NS);
